...
var nball = ['Your Hired!','Your Fired!']
...
socket.on('message',function(data){
	...
	TS = 0
    	...
    	trollboxMessagesSentThisMinute++           //  /  12   \
                                                   // |    |    |
                                                   // |9   |   3|
                                                   // |     \   |
                                                   // |         |
                                                   //  \___6___/
        thisDate = new Date();                     // tick-tock...
...
// ..............|XHHHHHHHHHHHHHHHH:::    :HHHHHHHHHHHHHHn   -.___.-.___..DrS.'
        ...
        if(data.msg.startsWith('fish')){
                sendMsg('🎣'+fish[Math.floor(Math.random()*fish.length)])
        }
        ...
        if(data.msg.startsWith('drink')){
                sendMsg('✨RPG Commands are currently in BATA. So this command is unavalable')
        }
        if(data.msg.startsWith('HP')){
                sendMsg('HP: 🖤🖤🖤🖤🖤🖤-- /n🚫RPG Commands are currently in BATA. So this command is unavalable')
        }
        ...
        if(data.msg.startsWith('help')){
                socket.emit('message','❔ Need Help? n/Functions: help, date, time, 8ball [A question], LMoS, ae <js>, quote, color <hex/Color name>, rename <name>, dice [number of sides], echo <text>, msgSent, MpM, MtM, joins, leaves, nlw, nickname, whoami, eat, exe <93program>, b <text>, zalgo <text>, ascii <image Url>, flip a coin, coin, say, 1ball [a queston], sleep\nRPG: Fish, Eat <food>, Drink <drink>, HP, Money, XP\n JW's Phrases: hi hello, Trophy, OwO, lol, bot, ping, zoombot, nutnat, is this a real chat?, 3ggnluc, eas, kao, shrug, soni, fort, robl, frank, Thank You, Fuck You\niwc, nat, iwt, llc, fith, hos, 2gre, ndk, nut, nat, jbh, gre, con, gav, awc, lw, ns, faa, about, reload, 93!, prefix')
        }
        ...
        if(data.msg.startsWith('zoombot')){           // /\  | <-- A red button
        ...
        if(data.msg.startsWith('Thank You')){                             //  O
                sendMsg('Your welcome!')                                  // -|- AHHH!
        }                                                                 //  /\
...
// I want you to do 2 passwords from there
// Once you do, i will curate a trophy in 93!Trophy
