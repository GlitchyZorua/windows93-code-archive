...
socket.on('message',function(data){
    ...
    if(data.msg.startsWith('93!')&&data.nick!=undefined){
        ...
        if(data.msg.startsWith('help')){
            socket.emit('message','❔ Need Help? For all commands, type 93!cmds, for tags, type 93!tags Verison: 7.6')
        }
        if(data.msg.startsWith('cmds')){
            socket.emit('message','❔ Funtions: help, quote, say, echo <text>, ae <js>, color <hex/Color name>, rename <name>, dice [number of sides], date, time, nlw, nickname, whoami, exe <93program>, b <text>, zalgo <text>, ascii <image Url>, 8ball [A question], flip a coin, coin, LMoS, msgSent, MpM, MtM, joins, leaves, sleep\nRPG: Fish, Eat <food>, Drink <drink>, HP, Money, XP, figet spinner\nhi, hello, Trophy, tags, cmds, 69ball [a question]')
        }
