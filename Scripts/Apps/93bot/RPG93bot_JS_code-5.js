...
socket.on('message',function(data){
    ...
    if(data.msg.startsWith('93!')&&data.nick!=undefined){
        ...
        if(data.msg.startsWith('help')){
                socket.emit('message','❔ Need Help? help, date, time, 8ball [A question], LMoS, ae <js>, quote, color <hex/Color name>, rename <name>, dice [number of sides], echo <text>, msgSent, MpM, MtM, joins, leaves, nlw, nickname, whoami, exe <93program>, b <text>, zalgo <text>, ascii <image Url>, flip a coin, coin, say, 1ball [a queston], sleep\n hi, hello, Trophy, OwO, lol, bot, ping, zoombot, nutnat, is this a real chat?, kao, shrug, soni, fort, robl, frank, Thank You, Fuck You, iwc, nat, iwt, llc, fith, hos, 2gre, ndk, nut, nat, jbh, gre, con, gav, awc, lw, ns, faa, about, reload, 93!, prefix, 3ggnluc, eas.n/RPG: Fish, Eat <food>, Drink <drink>, HP, Money, XP.')
        }
        ...
        if(data.msg.startsWith('3ggnluc')){
                sendMsg('YOU BOTH LOOK LIKE A FUCKING TRASH BIN, 3GG YOU ARE NOT EVEN A HACKER, YOU KNOW? YOU ARE JUST A EGG, A EXPIRED EGG, SO I'LL JUST STEP ON YOU AND YOU WILL CRUSH, LURICO IS A ASSHOLE TOO!')
        }
        ...
        if(data.msg.startsWith('eas')){
                sendMsg('EAS = Electronic Art's Shit')
        }
