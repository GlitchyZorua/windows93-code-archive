...

{
        ...
        menu: [
            {
                ...
                items: [
                    ...
                    { name: "---" }, // Separator
                    {
                        ...
                        action: function () {
                            if (GetSwatchTime) { // Ensure the element exists
                                if (sd == false) {
                                    sd = true
                                } else {
                                    sd = false
                ...
                items: [
                    {
                        name: "About & Credits",
                        action: function () {
                              $alert.info('Beat Time for Windows 93 v0.0.0\nTells the time in Beat Time (also know as Swatch Time\nBy GlitchyZorua\nSpecial Thanks: \'HOST\'')
                            }
                    },
                    { name: "---" }, // Separator
                    {
                        ...
                        action: function () {
                            ...
                            $exe('catex https://wiki.melonland.net/swatch_time')
                                ...
                                console.warn('Error occurred trying to open link in Catex. Attempting to open it in a iframe... Error is shown below')
                                console.error(error)
    ...
    var interval = setInterval(function () {
        b.children[0].innerText = '@' + GetSwatchTime(sd);
        showTime();
