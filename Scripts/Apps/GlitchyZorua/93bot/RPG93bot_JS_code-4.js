...
socket.on('message',function(data){
    ...
    if(data.msg.startsWith('93!')&&data.nick!=undefined){
        ...
        if(data.msg.startsWith('help')){
                socket.emit('message','❔ Need Help? help, date, time, 8ball [A question], LMoS, ae <js>, quote, color <hex/Color name>, rename <name>, dice [number of sides], echo <text>, msgSent, MpM, MtM, joins, leaves, nlw, nickname, whoami, eat, exe <93program>, b <text>, zalgo <text>, ascii <image Url>, flip a coin, coin, say, 1ball [a queston], sleep\nRPG: Fish, Eat <food>, Drink <drink>, HP, Money, XP\nhi, hello, Trophy, OwO, lol, bot, ping, zoombot, nutnat, is this a real chat?, 3ggnluc, eas, kao, shrug, soni, fort, robl, frank, Thank You, Fuck You\niwc, nat, iwt, llc, fith, hos, 2gre, ndk, nut, nat, jbh, gre, con, gav, awc, lw, ns, faa, about, reload, 93!, prefix')
        }
