  	...
  	if(data.msg.startsWith('!bot ')&&data.nick!=undefined){
        ...
        if(data.msg.startsWith('time')){
      	...
      	if(data.msg.startsWith('help')){
        	socket.emit('message','❔ Need Help? help, date, time, rename <name>, echo <text>, dice [number of sides], ae <js>, color <hex/Color name>, joins, leaves, msgSent, MpM, MtM, 8ball [A qustion], LMoS, hi, hello, OwO, Trophy, lol, iwt, iwc, nat, llc, lw82, shrug, bot, about, !bot, reload, hos, zoombot, fith, hos, ndk, con, kao, nutnat, nat, nut, sleep, soni, fort, robl, frank, ping, exe <93program>, nickname, whoami, b <text>, zalgo <text>, ascii <image Url>, coin, flip a coin')
        }
