...
socket.on('message',function(data){
    ...
    if(data.msg.startsWith('!bot ')&&data.nick!=undefined){
        ...
        if(data.msg.startsWith('help')){
                socket.emit('message','❔ Need Help? help, date, time, 8ball [A question], LMoS, ae <js>, quote, color <hex/Color name>, rename <name>, dice [number of sides], echo <text>, msgSent, MpM, MtM, joins, leaves, hi, hello, Trophy, OwO, lol, iwc, nat, iwt, llc, lw82, shrug, bot, ping, about, reload, !bot, fith, hos, 2gre, zoombot, ndk, nutnat, is this a real chat?, kao, con, nut, nat, jbh, soni, fort, robl, frank, nickname, whoami, exe<93program>, b <text>, zalgo <text>, ascii <image Url>, Thank You, Fuck You, flip a coin, coin, prefix, say, gre, con, gav, awc, lw, ns, faa, 1ball [a queston], sleep')
        }
        ...
        if(data.msg.startsWith('!bot')){
                sendMsg('👋Hi! type "!bot help" for the list of commands...')
        }
        ...
        if(data.msg.startsWith('nutnat')){
                sendMsg('Well did you know that Nut and Nat almost has the same name? pretty confusing and werid. Right?')
        }
        ...
        if(data.msg.startsWith('prefix')){
                socket.emit('message','✔ the prefix is !bot -- This setting CANNOT be changed')
        }
